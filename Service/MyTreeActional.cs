...
namespace Upload.Service
{
    internal class MyTreeActional
    {
        ...
        internal async Task Download(string folderPath, List<FileModel> fileModels)
        {
            try
            {
                CursorUtil.SetCursorIs(Cursors.WaitCursor);
                ...
                using (ProgressDialogForm form = new ProgressDialogForm($"Download to folder: {folderPath}"))
                {
                    await form.DoworkAsync(async (report, token) =>
                    {
                        ...
                        foreach (FileModel fileModel in fileModels)
                        {
                            ...
                            await DownloadFileModel(folderPath, fileModel);
                        }
        ...
        private async Task DownloadFileModel(string folderPath, FileModel fileModel)
        {
            await Task.Run(async () =>
            {
                ...
                else
                {
                    ...
                    await TranforUtil.Download(fileModel, fullPath, ConstKey.ZIP_PASSWORD);
                }
            });
        }
